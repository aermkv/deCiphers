...
function bg_rug() {
  ...
  for (let i = 0; i < n_l; i++){
    ...
    if(x < x_left_m){
      ...
      tS(x, y_mid, x, y_b, w(0.007), cs2[0], cs2[1], cs2[2], 85)
...
function checkerboard() {
  ...
  rotate(rt_an)
  //bM(BURN)
  let size = rnd(w(.003),w(.0045))
  let num_cols = w(1)/size/2
...
function bShape(pal,a,alpha) {
  ...
  //v(w(0.353)+ a,h(0.997));
  v(w(0.67)+ a,h(0.003));
  for (var y = h(0.003); y <= h(0.997); y += h(0.0005)){
...
function win(x_left_outside, x_left_inside, x_right_outside, x_right_inside, y_top, y_inside, y_bottom, pal, shadowBlur, sX, sY) {
  push()
...
function gradient_plate(x_left, x_break, y_top, gradient_width, gradient_height, gradient_height_2) {
  push()
  var c2 = color(gr_1[0], gr_1[1], gr_1[2], 125)
  var c3 = color(gr_3[0], gr_3[1], gr_3[2], 215)
  var xoff = 0
  var inc = .02
  let gp_nl = 800
...
function sailshape(x_l, y_b) {
  ...
  const gradient_mid_sail = sail_num_lines/6
  for (let i = 0; i < sail_num_lines; i++){
    let x = map(i, 0, sail_num_lines, x_l, w(0.997))
    if (i <= gradient_mid_sail){
        let inter = map(i, 0, gradient_mid_sail, 0, 1)
        c = lerpColor(b1_sail, b2_sail, inter)
      }else{
        let inter2 = map(i, gradient_mid_sail, sail_num_lines, 0, 1)
...
function rays() {
  ...
  for (let i = 0; i < num_sections; i++){
    let x0 = map(i, 0, num_sections, 0, w(1))
    let col = values[floor(rnd() * values.length)]
    let section_width = rnd(w(0.3),w(1.8))/num_sections
    //let section_width = width/num_sections // ALTERNATE
    let num_lines = 150
    for (let j = 0; j < num_lines; j++){
      ...
      let y1 = 0
      let y2 = h(1)
      tS(x, y1, x, y2, map(j,0,num_lines,w(.005),w(.05)), col[0], col[1], col[2], map(j,0,num_lines,0,15))
    }
  }
  pop()
}
...
function pnrm() {
  ...
  win(x_l_m, x_l_m + wWid, x_m, x_m - wWid, y_l, y_l+wWid, h(1), p.base, sV, -0.3*sV, 0)
  rcTrCut(x_m, x_r, y_r, h(0), p.aux)
  gradient_plate(x_l, x_l_m, h(0), x_m, h(1), y_l)
  push()
  pop()
  gradient_flat(x_m, y_r, x_r-x_m, (h(1)-y_r)/2, h(0.00055))
  win(x_l_m, x_l_m + wWid, x_m, x_m - wWid, y_l, y_l+wWid, h(1), p.light, 1.6*sV, -1.2*sV, 0) //maybe delete?
  pop()
...
function sails() {
  ...
  win(wxl, wxl + wWid, wxr, wxr - wWid, wTop, wTop+wWid, h(1) - barH, p.aux, w(0.015), -w(.005), -w(.002))
  noStroke()
  ctx.shadowOffsetX = 0;
  ctx.shadowOffsetY = 0;
  ctx.shadowBlur = 2.5*sV;
